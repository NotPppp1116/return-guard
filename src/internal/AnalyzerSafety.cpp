    ...
    bool VisitArraySubscriptExpr(clang::ArraySubscriptExpr* subscript) {
        ...
        if (type->isConstantArrayType()) {
            ...
            if (index_opt.has_value()) {
                if (index_opt->isNegative() || llvm::APSInt::compareValues(*index_opt, llvm::APSInt(size, true)) >= 0) {
...
private:
    bool is_stack_address(const clang::Expr* expression) {
        ...
        if (const auto* ref = llvm::dyn_cast<clang::DeclRefExpr>(expression)) {
            if (const auto* var = llvm::dyn_cast<clang::VarDecl>(ref->getDecl())) {
                if (var->getType()->isArrayType() && var->isLocalVarDecl() && !var->isStaticLocal()) {
                    return true;
                }
    ...
    void check_uaf_for_variable(const clang::VarDecl* var, const clang::Stmt* free_stmt, const clang::Expr* arg) {
        clang::CFG::BuildOptions options;
        options.PruneTriviallyFalseEdges = true;
        std::unique_ptr<clang::CFG> cfg = clang::CFG::buildCFG(
            function_, function_->getBody(), &context_, options);
        if (!cfg) {
            return;
        }
    ...
    void emit_oob_warning(const clang::ArraySubscriptExpr* expr, const llvm::APSInt& index, const llvm::APInt& size) {
        ...
        const unsigned diagnostic_id = diagnostics.getCustomDiagID(
            level,
            "returnguard safety: array index %0 is out of bounds (array size is %1)");
        
        diagnostics.Report(expr->getIdx()->getExprLoc(), diagnostic_id)
            << llvm::toString(index, 10, index.isSigned())
            << llvm::toString(size, 10, false);
    }
    ...
    void emit_stack_return_warning(const clang::ReturnStmt* stmt, const clang::Expr* val) {
        ...
        const unsigned diagnostic_id = diagnostics.getCustomDiagID(
            level,
            "returnguard safety: address of stack-allocated variable returned");

        diagnostics.Report(stmt->getReturnLoc(), diagnostic_id);
    }

    void emit_uaf_warning(const clang::Stmt* use_stmt, const clang::Stmt* free_stmt, const clang::VarDecl* var) {
        clang::DiagnosticsEngine& diagnostics = context_.getDiagnostics();
        const clang::DiagnosticsEngine::Level level =
            returnguard::options().fail_on_diagnostics
                ? clang::DiagnosticsEngine::Error
        ...
        const unsigned diagnostic_id = diagnostics.getCustomDiagID(
            level,
            "returnguard safety: use of memory after it was freed");
        const unsigned note_id = diagnostics.getCustomDiagID(
            clang::DiagnosticsEngine::Note,
            "returnguard safety: pointer was freed here");
